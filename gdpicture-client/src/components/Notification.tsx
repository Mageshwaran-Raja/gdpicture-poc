...
export default function Notification() {
    ...
    const checDivRef = useRef<HTMLDivElement>(null);
    const [isNotificationVisible, setNotificationVisible] = useState(false);
    const [notificationMessage, setNotificationMessage] = useState<any>(null);

    const connection = new HubConnectionBuilder()
        .withUrl('https://localhost:7049/signalr/notificationHub')
    ...
    const notificationHandler = () => {
        ...
        const checDiv = checDivRef.current;

        if (notification && checDiv) {
    ...
    return (
        <>
            <div className="py-8">
                <button onClick={() => notificationHandler()} className="focus:ring-2 focus:ring-offset-2 focus:ring-indigo-600 focus:outline-none  py-2 px-10 rounded bg-indigo-600 hover:bg-indigo-700 text-white">Open Modal</button>
            </div>
            <div className="w-full h-full bg-gray-800 bg-opacity-90 top-0 overflow-y-auto overflow-x-hidden fixed sticky-0" ref={checDivRef} id="chec-div">

                <div className="w-full absolute z-10 right-0 h-full overflow-x-hidden transform translate-x-0 transition ease-in-out duration-700" ref={notificationRef} id="notification">
                    <div className="2xl:w-4/12 bg-gray-50 h-screen overflow-y-auto p-8 absolute right-0">
                        <div className="flex items-center justify-between">
                            ...
                            <button role="button" aria-label="close modal" className="focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 rounded-md cursor-pointer" onClick={() => notificationHandler()}>
                        ...
                        <div className="w-full p-3 mt-4 bg-white rounded flex items-center">
                            <div tabIndex={0} aria-label="storage icon" role="img" className="focus:outline-none w-8 h-8 border rounded-full border-gray-200 flex items-center flex-shrink-0 justify-center">
                                <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <div className="pl-3 w-full flex items-center justify-between">
                                <p tabIndex={0} className="focus:outline-none text-sm leading-none text-gray-500">File is converted to PDF</p>
                                <a href={notificationMessage?.fileURL}><p tabIndex={0} className="focus:outline-none text-xs leading-3 cursor-pointer underline text-right text-indigo-700">Download</p></a>
                            </div>
                        </div>}
                        {/* <div className="w-full p-3 mt-4 bg-white rounded flex">
